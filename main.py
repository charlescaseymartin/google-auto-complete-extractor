...
def invalid_keyword_file():
    print('Invalid keyword file. Read help(-h/--help) menu for more')
    sys.exit(1)
...
def get_all_keywords(direct_keys: [str], key_file_dir: str):
    ...
    if direct_keys is None or len(direct_keys) < 1:
        print(f'get_all_keywords direct_keys: {direct_keys}')
        print('-k [--keywords] Invalid keywords provided.')
        sys.exit(1)
...
def parse_args():
    ...
    parser.add_argument('-k', '--keywords', required=False, nargs='*', help=key_help)
    parser.add_argument('-f', '--filename', required=False, nargs=1, help=key_file_help)
    parser.add_argument('-o', '--output', required=False, nargs=1, help=out_file_help)

    args = parser.parse_args()
    keyword_args = args.keywords
    keyword_file_arg = args.filename[0] if args.filename else None
...
if __name__ == '__main__':
    ...
    all_keywords = get_all_keywords(args.get('keywords'), args.get('keywords_file'))
    results = {}
